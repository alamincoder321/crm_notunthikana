...
class Model_Table extends CI_Model
{
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            /* total */
            (
                select received_sales + received_customer + invest_received + received_supplier + received_cash + bank_withdraw
            ) as total_in,
            (
                select paid_purchase + paid_customer + paid_supplier + paid_cash + invest_payment + bank_deposit + employee_payment
            ) as total_out,
            (
