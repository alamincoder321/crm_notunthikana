...
class Login extends CI_Controller
{
	...
	function procedureRegister()
	{
		...
		$checkUsername = $this->db->query("select * from tbl_user where User_Name = ?", $user)->num_rows();
		if ($checkUsername > 0) {
			$sdata['message'] = "Mobile Number Already Exists. Please Login";
		...
		$sdata['message'] = "Registration Success. Please Login";
		$this->load->view('login/login', $sdata);
	}
	...
	function procedure()
	{
		...
		if (isset($data)) {
			...
			if ($data->userstatus == 'a') {
				...
				$sdata['employeeId']    = $data->employeeId;
